...
## progress: the warning, above everything per Law 1). Lingering armed
## zones (Blightcaster) render at full progress — the complete ring is
## the "burning" state.
	...
	for hz: Dictionary in world.hazards:
		...
		if mode != Mode.HOSTILE_FILL:
			...
			if not zone_entry.is_empty() and not strip.is_empty():
				...
				# Primitive fallback: rim + sweeping arm arc (Law 8 holds
