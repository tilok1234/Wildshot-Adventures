...
| 9 | Entry 5 amended: the M-FX call is DECIDED (2026-07-27, curated projectile pack shipped + integrated). Remaining placeholder pieces: Nova cast-flash ring + generic muzzle/impact/kill flashes still procedural (flash_view), damage-number spam untouched by the pack, and the effect-density/opacity scaler + flash-reduction mode (CORE-50) not yet built. | `game/views/flash_view.gd`, options (M6) | M6 EffectLibrary pass + 9-row acceptance |
| 10 | Rusher slash (pattern 11) shares the fang sprite with Fanmaw's fan (pattern 13) — Law 3 wants per-family shapes; the pack's role table predates the slash decision. | `data/projectile_map.tres` (M5+) | Projectile-pack rev with a dedicated slash sprite, or designer accepts the share at the M6 9-row acceptance |
| 11 | DodgeBot primary policy models melee chasers as do-not-enter trigger bubbles (conservative); reactive windup-dodging (enter range, dodge on telegraph like a human) is not modeled — first_contact composition FAILS at 3.0 under this model and awaits designer adjudication. | `game/bots/dodge_policy.gd` (M5) | M7 secondary policies (orbit/axis-strafe/reactive) re-adjudicate compositions |
| 12 | Game-side blob-47 mask placement exists for hand-authored arena coverages (Forest Walk grass) — decision 2026-07-27 says resolution logic lives in WorldForge; suggested reading (flagged, designer note pending): temporary, retires if a WorldForge-exported arena replaces the hand-authored testbed. | `game/arena/arena_builder.gd` (M5+) | WorldForge-exported arena supersedes the hand-authored forest def, or designer records the grandfather note |
