## One-shot WINDOWED render probe (sl-0130): THE BANK PANEL at the
## stash station — bank rows (click to withdraw) + bag rows (click to
## deposit) in the one grammar, capacities in the headers. Captures
## (committed, read by eyes; desktop = screen crop per the rift_split
## discipline):
##   reports/bank_panel_audit_base.png
##   reports/bank_panel_audit_desktop.png
## Run WITHOUT --headless:
##   godot_console --path . --script tests/bank/bank_panel_probe.gd
extends SceneTree
...
func _run() -> void:
	...
	p.prev_pos = p.pos
	world.bank_cell = p.pos
	# Staged stash + bag: the two-list panel shows real rows.
	p.bank = PackedInt32Array([4, 1, 0, 5, 1, 0])
	p.bag = PackedInt32Array([2, 2, 0, 1, 0, 3])
	...
	shot.save_png(ProjectSettings.globalize_path("res://reports/bank_panel_audit_base.png"))
	print("bank_panel_probe: wrote bank_panel_audit_base.png")
	...
	if screen != null:
		...
		if _off(got_a, sig_a) or _off(got_b, sig_b):
			printerr("bank_panel_probe: desktop crop signature mismatch — NOT written")
		else:
			crop.save_png(
				ProjectSettings.globalize_path("res://reports/bank_panel_audit_desktop.png")
			)
			print(
				(
					"bank_panel_probe: wrote bank_panel_audit_desktop.png (%dx%d)"
					% [crop.get_width(), crop.get_height()]
				)
			)
	else:
		printerr("bank_panel_probe: screen_get_image unavailable — desktop NOT captured")
	quit(0)
