...
static func run(world: RefCounted) -> void:
	...
	for i in world.players.size():
		...
		if not p.dead and i < frames.size() and frames[i] != null:
			...
			# sl-0123: THE DRAG IS CUT — rift arenas move like every
			# other arena (the rift's pull lives in the LINE only:
			# drains, deep edge, tension). The mv==ZERO early-out is
			# universal again.
			if mv != Vector2.ZERO:
